 ...
 * returns. Filters are organized in chains, and are associated with {@link HttpContext} instances.
 *
 * <p>Each {@code HttpFilter} in the chain, invokes the next filter within its own {@link
 * #filter(HttpContext, FilterChain)} implementation. The final {@code HttpFilter} in the chain
 * invokes the applications exchange handler.
 */
@FunctionalInterface
public interface HttpFilter {
   ...
   */
  void filter(HttpContext ctx, FilterChain chain) throws Exception;
